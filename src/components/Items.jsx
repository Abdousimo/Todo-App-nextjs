...
const  Items = ({data,setter,courant,mode}) => {
 ...
 function handleComplited(id) {
  ...
  function handleDelete(id){
    ...
    return (
        <section className={`h-[50vh] w-full max-w-[350px] sm:max-w-[600px] md:max-w-[800px] overflow-auto rounded-t-lg py-5 ${mode ? 'bg-[#3C303C]' : 'bg-white'}`}>
                {  data.map((item,index) => (
                    <div key={index} className='w-full h-10 px-4 py-8 flex items-center justify-between border-b-2'>
                        <div className='flex text-center gap-8'>
                            <button onClick={()=> handleComplited(index)} className={`${mode ? 'text-white' : ''}`}>
                                {!item.isComplited ? <BiCheckbox size={35}/> : <BiCheckboxChecked size={35}/>}
                            </button>
                            <p className={`text-xl ${mode ? 'text-white' : ''}`}>{item.name}</p>
                        </div>
                        <button className={mode ? 'text-white' : ''} onClick={()=> handleDelete(index)}>
